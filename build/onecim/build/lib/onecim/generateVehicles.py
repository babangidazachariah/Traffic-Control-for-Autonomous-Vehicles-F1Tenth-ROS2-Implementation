...
class CIMAutonomousVehicle(Node):
    ...
    def __init__(self):
        ...
        self.crtVeh = self.create_timer(0.3, self.CreateVehicles)
        self.rdrwVeh = self.create_timer(0.3, self.ReDrawVehicles)
        self.rdrwVeh = self.create_timer(0.0083, self.PublishVehicles)
        self.idx = 0
        self.vehID = -1
        self.createdCount = 0
        self.removedCount = 0
    ...
    def DrawVehicles(self, vehicles):
      ...
      #['vehicleid', 'junctionid', 'approachleg', 'departleg', 'distfromstopline', 'point', 'arrivaltime', 'speed', 'priority', 'stoppedtime', 'waitingtime']  
        for vehicle in vehicles:
            self.drawnVehicles.markers.append(self.DrawVehicle(vehicle[5], vehicle[2], (0.0, 1.0, 0.0)))
    ...
    def PublishVehicles(self):
        ...
        if self.vehPubIndex == 1:
            for vehicle in self.eastSouth:
                if vehicle[4] <= self.MINSTOPLINEDIST and vehicle[4] >= -(self.MINSTOPLINEDIST * 0.65):
    ...
    def MoveVehicles(self, vehicles, path):
        ...
        for vehicle in vehicles:
            ...
            #record delay time
            if  vehicle[6] is not None and vehicle[4] <= -(self.MINSTOPLINEDIST * 0.65):
            ...
            if not self.tlsMSG is None:
                ...
                if path not in self.tlsMSG.pathids:
                    if not stopVehs:
                        ...
                        if (vehicle[4] >= 0 and vehicle[4] <= 1.0) or (vehicle[4] < 0 and vehicle[4] >= -0.8):   
    ...
    def AddVehicle(self, path):
        ...
        while dist < 5:
            ...
            i+=1
        self.vehID += 1    
        return [path + str(self.vehID), 'JC1', path, path, self.DistanceFromStopLine(path, (x,y)), (x,y), None, rnd.randint(0, 2), 1, None, None]
        #['vehicleid', 'junctionid', 'approachleg', 'departleg', 'distfromstopline', 'point', 'arrivaltime', 'speed', 'priority', 'stoppedtime', 'waitingtime']
    ...
    """"
        This part of the code assumes that communication is perfect. Thus, we record or update 
        vehicular data in the csv
        It consist of:
            def InsertNewRecord(self, msg):
            def DeleteVehicleFromIntersection(self, vehicleid, approachleg):
            def RecordTrafficData(self, msg):
        All of which are supposed to be part of the centrall controller    
    """  
    def InsertNewRecord(self, msg):
        """
        ...
        self.trafficData = self.trafficData[~((self.trafficData['vehicleid'] == msg[0]) & (self.trafficData['approachleg'] != msg[2]))]
        
        self.trafficData.loc[len(self.trafficData)] = {'vehicleid': msg[0], 
                                                        'junctionid': msg[1], 
                                                        'approachleg': msg[2], 
                                                        'departleg': msg[3],
                                                        'vehicletype': 1, 
                                                        'distfromstopline':msg[4], 
                                                        'point':msg[5], 
                                                        'arrivaltime': msg[6], 
                                                        'speed': msg[7], 
                                                        'priority': 0,
    ...
    def RecordTrafficData(self, msg): 
        ...
        curTime = time.time()
        try:
            self.trafficData = self.trafficData[curTime - self.trafficData['timestamp'] <= 60]
        ...
        if len(self.trafficData) > 0: #There is a record in it
            try:
                ...
                idx = self.trafficData[((self.trafficData['vehicleid'] == msg[0]) & (self.trafficData['approachleg'] == msg[2]))].index[0]
                
                if msg[4] > -self.MINSTOPLINEDIST/3 : #vehicle is yet past the intersection, thus, we update its data
                ...
                if idx is None: #Just to confirm that generated the error that hindered updates, which means vehicle is not in the dataframe
                    ...
                    if msg[4] > -self.MINSTOPLINEDIST/3.0: 
                        self.InsertNewRecord(msg)
                        print("Inserted vehicle: ", msg[0])
                    else:
                        self.DeleteVehicleFromIntersection(msg[0], msg[2])
