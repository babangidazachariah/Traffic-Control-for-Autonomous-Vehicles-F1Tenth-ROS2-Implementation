...
class FuzzyIntersectionManager:
    ...
    def CreateFIS(self):
        ...
        qLRange = np.arange(0, 51, 0.01) #Assuming a maximum of 50 vehicles at 4.5m length, means distance of 4.5 * 50 = 225m
        ...
        queueLength['short'] = fuzz.trimf(qLRange, [0, 3, 6])
        queueLength['long'] = fuzz.trimf(qLRange, [5, 9, 13])
        queueLength['verylong'] = fuzz.trimf(qLRange, [11, 16, 20])
        ...
        speedOut['low'] = fuzz.trimf(sORange, [0.3, 0.5, 0.7])
        speedOut['high'] = fuzz.trimf(sORange, [0.5, 0.9, 1.4])
        speedOut['veryhigh'] = fuzz.trimf(sORange, [1.3, 1.7, 2.0])
