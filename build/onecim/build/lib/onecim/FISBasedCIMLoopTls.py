...
class OneCIMFISTLS(Node):
    ...
    def SwitchTrafficSignal(self):
        ...
        if (self.signalDuration - duration) > 0 and (self.signalDuration - duration) < 2 and not self.startTime == 0.0 and not self.nextSchedulePerformed: # 5s to end of signal duration
            ...
            if len(self.trafficData) > 0:
                ...
                if appLeg in self.nextFlows:
                    altAppLeg = None
                    if appLeg == "NS1" and appLeg == "SN1":
                        altAppLeg = 'WE0'
                    elif appLeg == "WE1" and appLeg == "EW1":
                        altAppLeg = 'NE0'
                    elif appLeg == "NE0" and appLeg == "SW0":
                        altAppLeg = 'NS1'
                    elif appLeg == "ES0" and appLeg == "WN0":
                        altAppLeg = 'EW1'
        ...
        elif (((self.signalDuration - duration) > 2 ) and ((self.signalDuration - duration) <= 7)): #change green light to yellow
            self.TrafficLights(self.nextFlows,'yellow')
        elif (self.signalDuration - duration) <= 2: #Change traffic lights to RED-CLEARANCE (All-Red) Phase
