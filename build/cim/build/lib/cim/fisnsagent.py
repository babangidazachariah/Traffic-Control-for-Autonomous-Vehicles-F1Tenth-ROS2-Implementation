...
class FISCIMNsagent(Node):
    ...
    def __init__(self):
        ...
        self.absDistStopLine = 2000 #set to large amount to allow vehicle start from there location    
    ...
    def GetWaitRecordingStatus(self, future):
        result = future.result().result
        if result.recorded:
            self.isWaitingTimeRecorded = False


    def WaitingTimeRecorded(self, future):
        
        goalHandle = future.result()
        if not goalHandle.accepted:
            self.get_logger().info('Goal rejected :(')
            self.isWaitingTimeRecorded = False #That is, trafficInfo was not accepted for recording and should be sent for the current intersection
            return

        #self.get_logger().info('Goal accepted :)')
        self.isWaitingTimeRecorded = True #That is, trafficInfo was accepted for recording and shoul not be sent again for the current intersection
        self.GetWaitRecordingStatusFuture = goalHandle.get_result_async()
        self.GetWaitRecordingStatusFuture.add_done_callback(self.GetWaitRecordingStatus)
        
        
    def RecordWaitingTime(self):
        #Sends vehicle waiting time at an intersection to the traffic record and analysis node
        wtTimemsg = VehWaitTime.Goal()
        wtTimemsg.approachleg = self.approachLeg
        wtTimemsg.waittime = time.time() - self.initialTime
        self.trafficRecordClient.wait_for_server()
        self.sendTrafficInfoFuture = self.trafficRecordClient.send_goal_async(wtTimemsg)
        self.sendTrafficInfoFuture.add_done_callback(self.WaitingTimeRecorded)
    ...
    def RegisterPresence(self):
        ...
        regPresenceMsg.arrivaltime = float(self.initialTime)
    ...
    def TlsBasedTrajectoryPlanning(self, tlsmsg):
        ...
        """
        #print('TlsBasedTrajectoryPlanning tlsmsg.starttime: ', tlsmsg.starttime)
        ...
        if tlsmsg.starttime is not None: # and self.initialTime is not None:
            ...
            if self.approachLeg not in tlsmsg.pathids : #Check to ensure the received tls message is for the vehicle path - vehicle path scheduled
                ...
                #print('absDistStopLine: ', self.absDistStopLine)
                #if abs(self.absDistStopLine) < 1.5 and dur < (tlsmsg.duration - 3):
                
                #print('tlsmsg.duration: ', tlsmsg.duration, ' durtation: ', dur)
                if ((self.absDistStopLine > 0 and self.absDistStopLine < 1.5) or (self.absDistStopLine > 0  and self.absDistStopLine < 2.0 and ((tlsmsg.duration - dur) < 4))):
        ...
        if self.stopMe is True: #self.stopMe is True ==> This vehicle has the Red Signal
            ...
            for veh in self.vehAtCurJunction:
                if veh['distfromstopline'] < -0.5 and veh['approachleg'] != self.approachLeg: #A vehicle is already in the conflict zone, I should wait
                    #print('Opp Veh Dist: ', veh['distfromstopline'])
                    #print('Opp Veh appL: ', veh['approachleg'])
                    vehIn = True
            ...
            #print('vehIn: ', vehIn)
            if vehIn is True:
                self.speed = self.recmdedSpeed * 0.6
            else:
            ...
            #Reduce speed if steering angle is not greater than straight steering threshold
            if abs(self.steerAngle) > self.disparityExtender.GetStraightSteeringAngle():
                self.speed = 0.6 
            
        print('speed: ', self.speed) 
        #print('Recommended speed: ', self.recmdedSpeed) 
    ...
    def ProcessLidar(self, data):
        ...
        #publish drive command could be called from here
