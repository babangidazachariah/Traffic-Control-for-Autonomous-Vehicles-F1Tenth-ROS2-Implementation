...
class CIMAutonomousVehicle(Node):
    ...
    def __init__(self):
        ...
        self.dsrcSubscriber = self.create_subscription(IntersectionBSM, '/southpath', self.RecordTrafficData, 50) # qos_profile=rclpy.qos.qos_profile_system_default)
        ...
        #self.dsrcSubscriber = self.create_subscription(IntersectionBSM, '/westpath', self.RecordTrafficData, 200) # qos_profile=rclpy.qos.qos_profile_system_default)
    ...
    def WriteTrafficData(self, tlsmsg):
        ...
        if (((tlsmsg.duration - dur) > 2) and ((tlsmsg.duration - dur) < 3)):
            ...
            curTime = time.time()
            print("Initial: ", len(self.southTrafficData))
            #"""
            try:
                self.southTrafficData = self.southTrafficData[curTime - self.southTrafficData['timestamp'] <= 30]
            except IndexError as e:
                curTime = 0
            #"""
            trafficData = self.southTrafficData.copy() #Copy current self.trafficData
            self.southTrafficData.drop(self.southTrafficData.index)
            ...
            try:
                trafficData.to_csv('src/onecim/trafficData/southTrafficData.csv')
    ...
    def InsertNewRecord(self, msg):
        ...
        self.southTrafficData.loc[len(self.southTrafficData)] = {'vehicleid': msg.vehicleid, 
    ...
    def DeleteVehicleFromIntersection(self, vehicleid, approachleg):
        ...
        #Delete the vehicle
        self.southTrafficData = self.southTrafficData[~((self.southTrafficData['vehicleid'] == vehicleid) & (self.southTrafficData['approachleg'] == approachleg))]
        """
        #Compute the delay:
