...
class CIMAutonomousVehicle(Node):
    ...
    def __init__(self):
        ...
        self.dsrcPublisher = self.create_publisher(IntersectionBSM, '/dsrcmsg', 1000) # qos_profile=rclpy.qos.qos_profile_system_default)
         ...
         #Create Acknoledgement publisher
        self.dsrcAckPublisher = self.create_publisher(String[], '/ack', 100) # qos_profile=rclpy.qos.qos_profile_system_default)
    ...
    def WriteTrafficData(self, tlsmsg):
        ...
        if (((tlsmsg.duration - dur) > 2) and ((tlsmsg.duration - dur) < 3)):
            ...
            try:
                self.trafficData = self.trafficData[curTime - self.trafficData['timestamp'] <= 50]
    ...
    def InsertNewRecord(self, msg):
        ...
        #Send Acknowlegement to vehicle:
        self.dsrcAckPublisher.publish([msg.approachleg[:2], msg.vehicleid, 2]) #Approach handshake acknowledgement
    ...
    def DeleteVehicleFromIntersection(self, vehicleid, approachleg):
        ...
        self.dsrcAckPublisher.publish([approachleg, vehicleid, 4]) #Depart handshake acknowledgement
        """
        #Compute the delay:
