...
class OneCIMTLS(Node):
    ...
    def __init__(self):
        ...
        #self.dsrcmsSub = self.create_subscription(IntersectionBSM, '/dsrcmsg', self.RecordTrafficData, 1000)
        ...
        self.minPhaseDuration = 8 #Minimum phase duration assignable to a flow
    ...
    def StopRecordingReceiveTime(self):
        ...
        if diff >= self.simDuration: #
            path = self.recieveTime.to_csv('thirtyVehReceiveTime.csv') #write recorded waitTime to CSV file for analysis
            #print('Waiting Times file path: ',path)
            path = self.compTime.to_csv('thirtyVehCompTime.csv')
    ...
    def SwitchTrafficSignal(self):
        ...
        if (self.signalDuration - duration) >= 0 and (self.signalDuration - duration) < 1 and not self.startTime == 0.0 and not self.nextSchedulePerformed: # 5s to end of signal duration
            ...
            if len(self.trafficData) > 0:
                ...
                maxDistance = 0
                maxQueueLength = 0
                t = self.maxPhaseDuration
                chosenLegs = 'SN1'
                idx = None
                concFlows = self.GetConcurrentFlows('SW0', 'SW0')
                ...
                concFlows = self.GetConcurrentFlows(appLeg, appLeg)
                for flow in concFlows:
                    #Get Vehicles on the flow
                    tmpData = self.trafficData[self.trafficData['approachleg'] == flow]
                    if len(tmpData) > 0: #Meaning there are vehicles on this flow
                        #loop through the vehicles and 
                        for index, row in tmpData.iterrows():
                            apl = row['approachleg']
                            spd = row['speed']
                            pnt = row['point']
                            t0 = self.ComputePhaseDuration(apl, pnt, spd)
                            if  t0 < self.maxPhaseDuration: #t was initially set to self.maxPhaseDuration
                                if t0 > self.minPhaseDuration: # t0 > t and t0 < self.maxPhaseDuration:
                                    t = t0
                                else:
                                    t = self.minPhaseDuration 
                            else:
                                t = self.maxPhaseDuration
                                break

                #print('Not Empty:', self.trafficData)
                #print('concFlows: ', concFlows)
                self.nextFlows = concFlows
                self.nextSignalDuration = t if t != 0 else 8 #8s is the minimum time require by vehicles to safely utilize the intersection at 1m/s speed
                ...
                #print('Next Flow: ', self.nextFlows, ' Wait Time: ', maxArivalTime, ' Queue Len: ', maxQueueLength, ' Phase Dur: ', self.nextSignalDuration)
            else:
                #print('Empty:', self.trafficData)
                self.nextRecommendedSpeed = random.randint(1, 4)
    ...
    def RecordTrafficData(self, msg): 
        ...
        print('Len: ', len(self.trafficData), ' vehicleid: ', msg.vehicleid, 'recievetime: ', time.time())
        idx = None 
