...
class CIMAutonomousVehicle(Node):
    """
        ...
        One Intersection EWAgent Keypoints:
        Stop Line: (5.67632, -4.22149, 0)
        Safe Point after intersection: (6.11296, 3.86962, 0)
        End of Path:  Position(6.11725, 23.8719, 0)
        
        One Intersection SNAgent Keypoints:
        Stop Line: (1.01726, -0.787557, 0)
        Safe Point after intersection: (9.48962, -0.8483, 0)
        End of Path: (27.6704, -0.827079, 0)
        
        WE Stop line: (4.04208, 3.99244, 0)
        WE Safe point: (4.03338, -4.06445, 0)
        
        NS Stop line: (9.7611, 0.740437, 0)
        NS Safe point: (1.03482, 1.0365, 0)
        
        approach = {'EW': (5.67632, -4.22149), 'SN': (1.01726, -0.787557), 'WE': (4.04208, 3.99244), 'NS':(9.7611, 0.740437) }
        depart = {'EW':(6.11296, 3.86962), 'SN': (9.48962, -0.8483), 'WE': (4.03338, -4.06445), 'NS': (1.03482, 1.0365)}
        One Intersection EW Stopline to safe point distance:  8.102883161054466
        One Intersection SN Stopline to safe point distance:  8.9624875922289
    """
    ...
    def __init__(self):
        ...
        self.purePursuit = PurePursuitPlanner('/home/babangida/ros_ws/src/onecim/onecim/ew1intersectionwaypointsvelocityprofile.csv', 'wepath', 'wepoint')
        ...
        self.waitTimeFileName = 'onecimsnagent.csv' #FISHybridCIMewagent  STLSCIMewagent
    ...
    def TlsBasedTrajectoryPlanning(self, tlsmsg):
        ...
        print('distFromStopLine: ', self.distFromStopLine, ' Rem Duration: ', dur, ' Assined Dur: ', tlsmsg.duration)
        if tlsmsg.starttime is not None and self.arrivalTime is not None:
            ...
            if self.approachLeg not in tlsmsg.pathids : #Check to ensure the received tls message is for the vehicle path - vehicle path scheduled
                ...
                if ((self.distFromStopLine > 0 and self.distFromStopLine < 1.5)):
                        print('EW Agent @ stop line: distFromStopLine=', self.distFromStopLine, ' duration: ', dur)
                        self.speed = 0.
            ...
            if len(tmpTfData) > 0: #There are vehicles on the same lane
                ...
                for idx, row in tmpTfData.iterrows():
                    if row['distfromstopline'] < self.distFromStopLine: #meaning there is a vehicle before this vehicle at the intersection
                        ...
                        dist = self.CalculateDistance(row['point'][0], row['point'][1], self.currStopLinePoint[0], self.currStopLinePoint[1])
                        if dist <= self.minGap:
                            speed = 0.0
                        #else:
                        #    speed = self.recmdedSpeed
    ...
    def GetStopLineDistance(self, vehX, vehY, agentPath):
        ...
        if agentPath == 'NS':
            appLegs = ['SN', 'NS'] 
            deptLegs = ['SN','NS']
            stopLinePoints = [(1.01726, -0.787557), (9.7611, 0.740437)] 
        elif agentPath == 'WE':
            appLegs = ['WE', 'EW'] 
            deptLegs = ['WE', 'EW']
            stopLinePoints = [(5.67632, -4.22149),  (4.04208, 3.99244)] 
    ...
    def StartStopDSRCMsgs(self, poseX, poseY, appLeg, deptLeg):
        ...
        if self.atStopLine and -dist <= (-self.MINSTOPLINEDIST * 1.6):
        ...
        dist = self.CalculateDistance(poseX, poseY, 6.11725, 23.8719)
        if dist <= 1.5:
