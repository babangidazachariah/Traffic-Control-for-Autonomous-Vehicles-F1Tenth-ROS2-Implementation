...
class OneCIMFISTLS(Node):
    ...
    def __init__(self):
        ...
        #self.dsrcmsSub = self.create_subscription(IntersectionBSM, '/dsrcmsg', self.RecordTrafficData, 200)
        ...
        self.maxPhaseDuration = 25 #Maximum phase duration assignable to a flow
    ...
    def StopRecordingReceiveTime(self):
        ...
        if diff >= self.simDuration: #
            path = self.recieveTime.to_csv("thirtyVehFISReceiveTime.csv") #write recorded waitTime to CSV file for analysis
            #print('Waiting Times file path: ',path)
            path = self.compTime.to_csv('thirtyVehFISCompTime.csv')
    ...
    def TrafficLights(self, appLegs, color='green'):
        #print('appLegs: ', appLegs, ' Color: ', color)
        ...
        for pt in self.approachLegs:
            if pt.key in appLegs:
                x1, y1 = pt.x, pt.y
                #print('Setting', pt.key, ' to ', colorCode)
    ...
    def SwitchTrafficSignal(self):
        ...
        if (self.signalDuration - duration) >= 1 and (self.signalDuration - duration) < 2 and not self.startTime == 0.0 and not self.nextSchedulePerformed: # 5s to end of signal duration
            ...
            if len(self.trafficData) > 0:
                ...
                idx = 0
                appLeg = 'SW0'
                tmpTFData = self.trafficData.copy()
                
                curTime = time.time()
                try:
                    tmpTFData = tmpTFData[curTime - self.trafficData['stoppedtime'] > 0]
                except IndexError as e:
                    curTime = 0
                if len(tmpTFData) > 0: #Meaning len is not 0 ==> There are waiting vehicle(s)
                    #Sort based on stoppedtime which is used for to determine waiting time
                    tmpTFData = tmpTFData.sort_values(by='stoppedtime', ascending=False)
                    print('Using time: ')
                    #Get waiting time and approach leg
                    idx = len(tmpTFData)-1
                    # Reset index after dropping duplicates
                    tmpTFData = tmpTFData.reset_index(drop=True)
                    maxWaitingTime = curTime - tmpTFData.at[idx, 'stoppedtime']
                    appLeg = tmpTFData.at[idx, 'approachleg']
                else: #There are no waiting vehicles, we go with arrivaltime
                    tmpTFData = self.trafficData.copy() 
                    #Sort based on arrivaltime which is used for to determine waiting time
                    tmpTFData = tmpTFData.sort_values(by='arrivaltime', ascending=False)
                    idx = len(tmpTFData)-1
                    #Get waiting time and approach leg
                    maxWaitingTime = curTime - tmpTFData.at[idx, 'arrivaltime']
                    appLeg = tmpTFData.at[idx, 'approachleg']
                
                tmpTFData['waitingtime'] = curTime - tmpTFData['stoppedtime']
                tmpTFData['timestamp'] = curTime - tmpTFData['timestamp']
                #print('',)      
                tmpTFData.to_csv('strafficData.csv')
                """
                ...
                if not math.isnan(stoppedIdx): #There is a vehicle waiting, stoppedIdx is not NaN
                    idx = stoppedIdx
                    maxWaitingTime = time.time() - self.trafficData.loc[idx]['stoppedtime'] #Get the max waiting time
                    print('Using waitTime')
                    
                elif not math.isnan(arrivalIdx):
                    idx = arrivalIdx
                    maxWaitingTime = time.time() - self.trafficData.loc[idx]['arrivaltime'] #Get the max waiting time
                    print('Using arrivalTime')
                print('Vehicle with max wTime: ', self.trafficData.loc[idx]['vehicleid'])
                #Confirm that maxWaitingTime has been set, otherwise set it to zero
                if maxWaitingTime is None:
                    maxWaitingTime = 0.0
                
                #get approach leg to be scheduled - having the vehicle with the longest waiting time
                appLeg = self.trafficData.loc[idx]['approachleg']
                ...
                """
                #This part is not required as we want schedule based on waiting time and arrival time
                ...
                if appLeg in self.nextFlows:
                    altAppLeg = None
                    if appLeg == "NS1" or appLeg == "SN1" or appLeg == "SE1" or appLeg == "NW1":
                        altAppLeg = 'WE0'
                    elif appLeg == "WE1" or appLeg == "EW1" or appLeg == "EN1" or appLeg == "WS1"  :
                        altAppLeg = 'NE0'
                    elif appLeg == "NE0" or appLeg == "SW0":
                        altAppLeg = 'NS1'
                    elif appLeg == "ES0" or appLeg == "WN0":
                        altAppLeg = 'EW1'
                     
                    print('appLeg: ', appLeg, ' altappLeg ',altAppLeg)
                    ...
                    if len(tmpTfData) > 0:
                        tmpTfData = tmpTfData.sort_values(by='timestamp', ascending=False)
                        appLeg = tmpTfData['approachleg'].iloc[0] #set the approach leg to the first element in the temp trafficData
                        maxWaitingTime = time.time() - tmpTfData.loc[len(tmpTfData)]['arrivaltime'] # Reset the waiting to the waiting time of the chosen vehicle
                #else:
                """
                ...
                flows = self.GetConcurrentFlows(appLeg, appLeg)
                
                for flow in flows:
                    #Retrieve vehicles on the same flow as the vehicle with highest waiting time
                    vehicles = self.trafficData[self.trafficData['approachleg']== flow]
                    ...
                    if len(vehicles) > 0:
                        for idx, vehicle in vehicles.iterrows():
                            dist = vehicle['distfromstopline'] #self.DistanceFromStopLine(vehicle['approachleg'], vehicle['point'])
                ...
                self.nextFlows = flows
                #print('Next Flow: ', self.nextFlow, ' Wait Time: ', maxArivalTime, ' Queue Len: ', maxQueueLength, ' Phase Dur: ', self.nextSignalDuration)
                print('Params: ',maxWaitingTime, maxQueueLength, meanSpeed, maxDistance)
                print('Phase Duration',self.nextSignalDuration)
                end = time.time()
                ...
                self.nextRecommendedSpeed = random.randint(1, 4)
                self.nextSignalDuration = 7 #5 + 2 (red clearance phase
                self.StartFromInitial()
                
        elif (((self.signalDuration - duration) > 3 ) and ((self.signalDuration - duration) <= 7)): #change green light to yellow
            #print('We are Setting traffic lights to Yellow ', (self.signalDuration - duration))
            ...
            if self.nextSignalDuration == 7 or self.nextFlows is None:
                #print('self.nextSignalDuration == 7: Restarting From Initial')
...
def main(args=None):
    ...
    print("FISBased CIM and Traffic Lights System Node")
    tls = OneCIMFISTLS()
    rclpy.spin(tls)
