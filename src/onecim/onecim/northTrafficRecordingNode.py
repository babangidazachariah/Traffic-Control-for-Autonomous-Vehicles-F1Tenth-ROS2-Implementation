...
class CIMAutonomousVehicle(Node):
    ...
    def __init__(self):
        ...
        self.dsrcSubscriber = self.create_subscription(IntersectionBSM, '/northpath', self.RecordTrafficData, 50) # qos_profile=rclpy.qos.qos_profile_system_default)
    ...
    def WriteTrafficData(self, tlsmsg):
        ...
        if (((tlsmsg.duration - dur) > 2) and ((tlsmsg.duration - dur) < 3)):
            ...
            curTime = time.time()
            print("Initial: ", len(self.northTrafficData))
            #"""
            try:
                self.northTrafficData = self.northTrafficData[curTime - self.northTrafficData['timestamp'] <= 30]
            except IndexError as e:
                curTime = 0
            #"""
            trafficData = self.northTrafficData.copy() #Copy current self.trafficData
            self.northTrafficData.drop(self.northTrafficData.index)
            ...
            try:
                trafficData.to_csv('src/onecim/trafficData/northTrafficData.csv')
    ...
    def InsertNewRecord(self, msg):
        ...
        self.northTrafficData.loc[len(self.northTrafficData)] = {'vehicleid': msg.vehicleid, 
    ...
    def DeleteVehicleFromIntersection(self, vehicleid, approachleg):
        ...
        #Delete the vehicle
        self.northTrafficData = self.northTrafficData[~((self.northTrafficData['vehicleid'] == vehicleid) & (self.northTrafficData['approachleg'] == approachleg))]
        """
        #Compute the delay:
