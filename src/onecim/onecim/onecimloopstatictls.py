...
class OneCIMTLS(Node):
    ...
    def __init__(self):
        ...
        self.minPhaseDuration = 7 #Minimum phase duration assignable to a flow
        self.maxPhaseDuration = 27 #Maximum phase duration assignable to a flow
    ...
    def SwitchTrafficSignal(self):
        ...
        if (self.signalDuration - duration) >= 0 and (self.signalDuration - duration) < 1 and not self.startTime == 0.0 and not self.nextSchedulePerformed: # 5s to end of signal duration
            ...
            self.nextSignalDuration = self.maxPhaseDuration
...
def main(args=None):
    ...
    print("Static CIM and Traffic Lights System Node")
    tls = OneCIMTLS()
    rclpy.spin(tls)
