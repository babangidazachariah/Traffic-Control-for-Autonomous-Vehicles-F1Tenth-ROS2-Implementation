...
class CIMAutonomousVehicle(Node):
    ...
    def __init__(self):
        ...
        self.dsrcSubscriber = self.create_subscription(IntersectionBSM, '/eastpath', self.RecordTrafficData, 50) # qos_profile=rclpy.qos.qos_profile_system_default)
    ...
    def WriteTrafficData(self, tlsmsg):
        ...
        if (((tlsmsg.duration - dur) > 2) and ((tlsmsg.duration - dur) < 3)):
            ...
            curTime = time.time()
            print("Initial: ", len(self.eastTrafficData))
            #"""
            try:
                self.eastTrafficData = self.eastTrafficData[curTime - self.eastTrafficData['timestamp'] <= 30]
            except IndexError as e:
                curTime = 0
            #"""
            trafficData = self.eastTrafficData.copy() #Copy current self.trafficData
            self.eastTrafficData.drop(self.eastTrafficData.index)
            ...
            try:
                trafficData.to_csv('src/onecim/trafficData/eastTrafficData.csv')
    ...
    def InsertNewRecord(self, msg):
        ...
        self.eastTrafficData.loc[len(self.eastTrafficData)] = {'vehicleid': msg.vehicleid, 
    ...
    def DeleteVehicleFromIntersection(self, vehicleid, approachleg):
        ...
        #Delete the vehicle
        self.eastTrafficData = self.eastTrafficData[~((self.eastTrafficData['vehicleid'] == vehicleid) & (self.eastTrafficData['approachleg'] == approachleg))]
        """
        #Compute the delay:
