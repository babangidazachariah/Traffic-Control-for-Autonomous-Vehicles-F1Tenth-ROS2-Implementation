...
from onecim.BenchmarkFIS import ObservationModule, DecisionModule

from rclpy.action import ActionServer

class OneCIMBenchMarkTLS(Node):
    """
        OneCIMTlsMsg.msg
    ...
    def __init__(self):
        ...
        self.observationFIS = ObservationModule()
        self.decisionFIS = DecisionModule()
    ...
    def SwitchTrafficSignal(self):
        ...
        maxArivalTime = 0
        if (self.signalDuration - duration) >= 14 and (self.signalDuration - duration) < 15 and not self.startTime == 0.0 and not self.nextSchedulePerformed: # Determine Phase Extension
            self.nextSchedulePerformed = True
            #Read traffic Data and compute traffic intensity of the current and next phase as well as queue length of current phase
            rdy = False
            ...
            for cPhase in self.curPhase:
                curPTraffic = self.trafficData[( (self.trafficData['approachleg'] == cPhase))]
                ql += len(curPTraffic)
            #Traffic params of current phase
            aRc = ((ql)/(len(self.curPhase)*(self.signalDuration - duration))) #Traffic Arrival Rate
            qLc = ql/len(self.curPhase) #Traffic count
            
            ql = 0
            print(self.nextPhase)
            for cPhase in self.nextPhase:
                curPTraffic = self.trafficData[( (self.trafficData['approachleg'] == cPhase))]
                ql += len(curPTraffic)
            
            #Traffic params of current phase
            aRn = ((ql)/(len(self.nextPhase)*(self.signalDuration - duration))) #Traffic Arrival Rate
            qLn = ql/len(self.nextPhase) #Traffic count
            
            tIc = self.observationFIS.EvaluateFis(qLc, aRc)
            tIn = self.observationFIS.EvaluateFis(qLn, aRn)
            
            phaseExtension = self.decisionFIS.EvaluateFis(tIc, tIn)
            self.signalDuration += phaseExtension
            print('Extended Green Phase for ', self.nextFlows, ' compare with ', self.curPhase)
        elif (self.signalDuration - duration) >= 0 and (self.signalDuration - duration) < 1 and not self.startTime == 0.0 and self.nextSchedulePerformed: # 5s to end of signal duration
            ...
            if self.phasePointer > 3:
                self.phasePointer = 0
            self.nextPhase = self.GetConcurrentFlows(self.phaseSequence[self.phasePointer],self.phaseSequence[self.phasePointer])
...
def main(args=None):
    ...
    print("Benchmark Controller (Zou et al. 2023) for Traffic Lights System")
    tls = OneCIMBenchMarkTLS()
    rclpy.spin(tls)
