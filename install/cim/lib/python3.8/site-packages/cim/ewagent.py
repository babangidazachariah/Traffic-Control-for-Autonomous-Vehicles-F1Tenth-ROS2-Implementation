...
class CIMAutonomousVehicle(Node):
    ...
    def ScheduleTraffic(self, dsrcmsg):
        ...
        """
        #print(dsrcmsg.approachleg, ' :: ', self.approachLeg)
            ...
            if dsrcmsg.vehicleid != self.vehicleID: #Check to ensure the received DRSC message is not from the vehicle
                ...
                if dsrcmsg.approachleg[-2:] == self.approachLeg[-2:] : #Both vehicles are approaching or around the same intersection
                    ...
                    if dsrcmsg.initialtime > self.initialTime or dsrcmsg.speed > self.speed : #the other vehicle arrived before or has higher speed than this vehicle
                        ...
                        #if abs(self.absDistStopLine) < 1  and (dsrcmsg.distfromstopline > (-self.MINSTOPLINEDIST *0.5)) and dsrcmsg.distfromstopline != 1000:
                        if ((self.absDistStopLine > 0 and self.absDistStopLine < 1.5)): #if abs(self.absDistStopLine) < 1 and dsrcmsg.distfromstopline < -0.7:
                            ...
                            print('West-East Agent at stop line: absDistStopLine=', self.absDistStopLine, ' other Veh Dist: ',dsrcmsg.distfromstopline)
                            self.speed = 0.
    ...
    def GetFieldOfView(self, ranges, deno):
        """"
            Slices the Lidar ranges to focus on the front field of view (135 : -135) degrees 
            in which case deno = 8 ==> 270 degree field of view (FOV).
            when deno == 12, field of view is (90 : -90) which is 180 degrees FOV.
            Note: measurement is done from the center of the lidar vertical axis
        """
        #preprocess to remove NAN and INF readings
        
        # remove quadrant of LiDAR directly behind us
        eighth = int(len(ranges)/deno)
    ...
    def CheckObstacle(self, scan, lim=0.5):
        obsScan = np.where(scan < lim, True, False)
        #print(obsSubScan)
    ...
    def ProcessLidar(self, data):
        ...
        fovScan = self.GetFieldOfView(data.ranges, 8)
        adjSpeed = self.speed
        if self.CheckObstacle(fovScan, 0.5):
...
def main(args=None):
    ...
    print("Cooperative Intersection Management Aware Controller Node")
