...
class FISCIMEwagent(Node):
        ...
        if tlsmsg.starttime is not None : # and self.initialTime is not None:
            ...
            if self.approachLeg not in tlsmsg.pathids : #Check to ensure the received tls message is for the vehicle path - vehicle path scheduled
                ...
                if ((self.absDistStopLine > 0 and self.absDistStopLine < 1.5) or (self.absDistStopLine > 0 and self.absDistStopLine < 2.0 and ((tlsmsg.duration - dur) <= 4))):
                    print('EW Agent @ stop line: absDistStopLine=', self.absDistStopLine, ' duration: ', dur)
                    self.speed = 0.
                    self.stopMe = True
            else:
    ...
    def StartStopDSRCMsgs(self, poseX, poseY, appLeg, deptLeg):
        ...
        if self.atStopLine and -dist <= (-self.MINSTOPLINEDIST * 2):
            ...
            #print('', self.isWaitingTimeRecorded)
            self.checkNextIntersection = True
    ...
    def PreProcessLidarScan(self, ranges, kind='linear', const=-10):
        ...
        if const == -10:
            ...
            validIdx = np.where(np.isfinite(arr))[0]
            invalidIdx = np.where(np.isnan(arr) | np.isinf(arr))[0]
            ...
            f = interp.interp1d(angles[validIdx], arr[validIdx], kind='linear', fill_value='extrapolate')
            arr[invalidIdx] = f(angles[invalidIdx])
        else:
